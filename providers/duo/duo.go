...
var (
	...
	// ErrCouldNotFindDuoForm will be returned if, during the Duo v4 Frameless flow, a form that we are expecting to exist to POST back to the caller no longer exists.
	//
	// This indicates that either an unexpected response was received from Duo - probably that the API changed.
	ErrCouldNotFindDuoForm = errors.New("could not find Duo form")
)
